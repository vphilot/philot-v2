...
<template>
  <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch w-full">
    <h1 class="sr-only">
      Posts
    </h1>
    <div v-for="post in posts" :key="post.name" class="mb-4 md:mb-8">
      <router-link :to="post.path">
        <div>
          <Icon :icon="post.icon" width="64" class="w-8 h-8 mb-2 text-pink-500 dark:text-green-500" />
          <h2 class="text-3xl lg:text-5xl font-extralight md:font-thin tracking-wide mb-2 md:mb-4 lg:leading-snug">
            {{ post.name }}
          </h2>
